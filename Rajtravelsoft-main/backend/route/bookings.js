...
router.put("/:id/driver",requireAdmin, async (req, res) => {
  try {
...
router.post("/booked-completed",requireAdmin, async (req, res) => {
  try {
   // req.admin is already verified in requireAdmin middleware
...
router.get("/all-payments-admin",requireAdmin, async (req, res) => {
...
router.get("/client/:email",requireAdmin, async (req, res) => {
  try {
...
router.put("/update-offer/:id", requireAdmin, async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.id)
    if (!booking) return res.status(404).json({ message: "Booking not found" })
...
router.put("/use-offer/:id",requireAdmin, async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.id)
    if (!booking) return res.status(404).json({ message: "Booking not found" })
...
router.put("/:bookingId/payments/:paymentId/view",requireAdmin, async (req, res) => {
  try {
