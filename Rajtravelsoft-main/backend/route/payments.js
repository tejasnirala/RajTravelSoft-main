...
// Add a new payment
router.post("/:bookingId", authMiddleware,adminOnly, async (req, res) => {
  try {
...
router.put("/:bookingId/:paymentId", authMiddleware,adminOnly, async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.bookingId)
    if (!booking) {
      return res.status(404).json({ message: "Booking not found" })
...
router.get("/:bookingId/pdf/:action", authMiddleware,adminOnly, async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.bookingId);
    if (!booking) {
      return res.status(404).json({ message: "Booking not found" });
...
router.get("/:bookingId/whatsapp", authMiddleware,adminOnly, async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.bookingId);
    ...
    const lines = [
      ...
      , // Add the PDF URL to the message
    ];
