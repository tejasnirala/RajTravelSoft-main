...
import { faEye, faTrash, faTimes, faUserPlus, faFilter, faPlus, faTimesCircle } from "@fortawesome/free-solid-svg-icons" // Removed unused icons for status actions
...
const InquiryManagement = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    endDate: "",
    message: "",
    assignedTo: "", // New filter for assigned user (admin only)
  ...
  const [newInquiryData, setNewInquiryData] = useState({
    ...
    message: "",
    other: "",
    car: { type: "", title: "" } // Optional car details
    // status not needed here - defaults on backend
  })
  ...
  useEffect(() => {
    ...
    // Listen for new inquiry events
    socketRef.current.on('newInquiry', (newInquiry) => {
      console.log('Received new inquiry:', newInquiry)

      // Show toast notification
      toast.info(`📩 New Inquiry from ${newInquiry.name}`, {
        autoClose: 5000,
        position: "top-right"
      })
  ...
  // Updated: getStatus now uses inquiry.status
  const getStatus = (inquiry) => {
    return inquiry.status || 'pending' // Fallback for old data
  }

  // Updated: Status badge color based on status
  ...
  const applyFilters = () => {
    ...
    // Updated: Status filter using new statuses
    if (filters.status) {
  ...
  // Multi-select handlers - unchanged
  ...
  const handleBulkAssign = async () => {
    ...
    try {
      ...
      const assignPromises = selectedInquiries.map(id =>
        axios.put(`${API_BASE}/api/inquiries/${id}/assign`, { assignedTo: bulkAssignedTo }, { withCredentials: true })
      )
  ...
  // Updated: Handle status update - for cancel, open modal
  const handleUpdateStatus = async (inquiryId, newStatus) => {
    ...
    try {
      await axios.put(`${API_BASE}/api/inquiries/${inquiryId}/status`, { status: newStatus }, { withCredentials: true })
  ...
  const handleCancelInquiry = async () => {
    ...
    try {
      await axios.put(`${API_BASE}/api/inquiries/${cancelInquiryId}/status`, {
        status: 'cancelled',
  ...
  const handleDelete = async (inquiryId) => {
    ...
    try {
      await axios.delete(`${API_BASE}/api/inquiries/${inquiryId}`, {
        withCredentials: "true"
      })
  ...
  const handleAssign = async (inquiryId, assignedTo) => {
    ...
    try {
      await axios.put(`${API_BASE}/api/inquiries/${inquiryId}/assign`, { assignedTo },
        { withCredentials: true })
  ...
  // New Inquiry Handlers - unchanged (status defaults)
  ...
  const handleAddNewInquiry = async (e) => {
    ...
    setIsAdding(true);
    try {
      await axios.post(`${API_BASE}/api/inquiries`, newInquiryData, {
        withCredentials: true
      });
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="w-[100%] mx-auto bg-white rounded-xl shadow-lg p-6">
        ...
        {/* Filter Form - Updated status options */}
        {showFilters && (
          <div className="mb-8 bg-gray-50 p-6 rounded-lg shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {/* Assigned To Filter for Admin - unchanged */}
              {isAdminRole && (
                <div>
        ...
        {/* Bulk Actions for Admin - unchanged */}
        ...
        {/* Inquiry Table - Updated status display and actions */}
        {loading ? (
                  ...
                  return (
                    <tr key={inquiry._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap flex gap-2">
                        ...
                        {/* Updated: Status select dropdown - only for non-user roles */}
                        {isNonUserRole && (
                          <select
                            value={status}
                        ...
                        {/* Quick create booking button - unchanged */}
                        <button
                          onClick={() => {
                            const params = new URLSearchParams();
                            if (inquiry.name) params.set('name', inquiry.name);
                            if (inquiry.email) params.set('email', inquiry.email);
                            if (inquiry.mobile) params.set('phone', inquiry.mobile);
                            // navigate to /page with query params
                            window.location.href = `/pending?${params.toString()}`;
                          }}
                          title="Create booking from inquiry"
                          className="flex items-center justify-center w-8 h-8 rounded-full bg-green-100 hover:bg-green-200 transition-all duration-200 cursor-pointer"
                        >
        ...
        {/* Modal for viewing inquiry details - Updated to show status and cancel reason if cancelled */}
        {selectedInquiry && (
                ...
                <div className="space-y-3">
                  ...
                  {selectedInquiry.other && (
                    <div>
                      ...
                      <p className="whitespace-pre-wrap text-gray-700 mt-1 p-3 bg-gray-50 rounded">{selectedInquiry.other}</p>
                    </div>
                  )}
        ...
        {/* Add New Inquiry Modal - unchanged */}
        {showAddModal && isNonUserRole && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold text-gray-900">Add New Inquiry</h3>
                  <button
                ...
                <form onSubmit={handleAddNewInquiry} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Other (Code)</label>
                    <input
                      type="text"
                      name="other"
                      value={newInquiryData.other}
                      onChange={handleNewInquiryChange}
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                      disabled={isAdding}
                    />
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Car Type (Optional)</label>
                    <input
                      type="text"
                      name="car.type"
                      value={newInquiryData.car.type}
                      onChange={handleNewInquiryChange}
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                      disabled={isAdding}
                    />
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Car Title (Optional)</label>
                    <input
                      type="text"
                      name="car.title"
                      value={newInquiryData.car.title}
                      onChange={handleNewInquiryChange}
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                      disabled={isAdding}
                    />
