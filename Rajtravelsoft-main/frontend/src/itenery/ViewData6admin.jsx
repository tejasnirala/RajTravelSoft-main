...
const Viewdata6 = ({ id, autoDownload, onDownloadComplete }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoadings(true);
        try {
            await axios.post(`${BASE_URL}/api/inquiries`, form);
                        ...
                        <div className="w-full lg:w-1/2 order-2 lg:order-1">
                            ...
                            <div className="mt-6 flex gap-4">
                                ...
                                <button
                                    ...
                                    className={`w-full flex items-center justify-center gap-2 py-2 rounded-lg transition-colors ${emailLoading
                                ...
                                <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                                    {bookingData.itineraryData.descriptions.map((desc, i) => (
                                        <div key={i} className="flex items-start gap-2 sm:gap-3 p-3 sm:p-4 bg-gray-50 rounded border border-gray-200">
                                            ...
                                            <div className="text-gray-700 leading-relaxed font-medium text-sm sm:text-base ql-editor" dangerouslySetInnerHTML={{ __html: desc }} />
                                        </div>
                                    ))}
                                </div>
                            )}
                            <div className="space-y-8 sm:space-y-12">
                                {(booking.itinerary || []).map((item, index) => (
                                    <div key={`it-day-${index}`} className="flex flex-col sm:flex-row gap-3">
                                        <div className="md:w-22 md:flex-col gap-4 md:gap-0 flex sm:w-22 sm:text-right text-left order-1">
