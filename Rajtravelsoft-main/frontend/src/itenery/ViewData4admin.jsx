...
// Solid icons
import { faPrint, faGlobe, faFileContract, faUndoAlt, faPlane, faCircleInfo, faLocationArrow, faWallet, faCoffee, faUtensils, faMoon, faHotel, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
...
const Viewdata4 = ({ id, autoDownload, onDownloadComplete }) => {
    ...
    const [isCustomerMode, setIsCustomerMode] = useState(false); // ← YE ADD KARO
    ...
    const handlePrint = async (customerMode = false) => {
        ...
        try {
            ...
            if (customerMode) {
                pageUrl.searchParams.set("noprice", "1"); // ← Important parameter
            } else {
                pageUrl.searchParams.delete("noprice");
            }
        ...
        const fetchBooking = async () => {
            try {
                ...
                if (data?.theme?.link) {
                    ...
                    setItineraryUrl(`https://tour.rajasthantouring.in/admin/${data.theme.link}/${bookingId}`);
                } else {
...
${addonsList ? `\nExtra - Add-ons:\n${addonsList}\n` : ""}
Received Amount: ₹${firstSuccessPayment?.amount || 0}/-
...
Travel Team`;
  ...
  <ul>
    ...
    ${festivalOfferHtml}
    <li><b>Received  Amount:</b> ₹${firstSuccessPayment?.amount || 0}/-</li>
                ...
                <section className="pt-6 md:pt-8">
                    <div className="flex flex-col md:flex-row items-start justify-between flex-wrap gap-6 md:gap-0">
                        <div className="w-full md:order-1 order-2 md:w-1/2">
                            ...
                            <div className="flex flex-wrap gap-2 my-2  items-center justify-start  w-full  ">
                                <button
                                    className={`px-4 py-2 cursor-pointer bg-blue-500 text-white rounded-md flex items-center justify-center gap-2 text-sm font-medium transition-all w-full sm:w-auto ${emailLoading ? "cursor-not-allowed opacity-50" : ""}`}
                                ...
                                >
                                    ...
                                    <span>{emailLoading ? "Sending..." : "Email"}</span>
                                </button>
                                <button
                                    className="px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition-all text-sm flex items-center justify-center gap-2 font-medium w-full sm:w-auto"
